...
def get_field_values(library, field_type):
    ...
    if field_type == 'SpeedUp-Options':
        #collection=library+field_type+'fieldvalue'
        collection = library.lower() + 'version(s)fieldvalue'
    frame = reader.read_collection(database=AUX_DB, collection=collection)
...
def is_multi_selection(library, field_type):
    ...
    _ = library #No need to query db. Same always
...
def get_specs(hardware_id, rocm):
    ...
    host_df = reader.read_collection(database=AUX_DB, collection=collection_host)
    device_df = reader.read_collection(database=AUX_DB, collection=collection_device)
...
def get_field_types(library):
    ...
    field_types = reader.read_collection(database=AUX_DB, collection=collection)
...
def get_registered_libraries():
    ...
    registered_libraries = reader.read_collection(database=AUX_DB, collection='libraries')
